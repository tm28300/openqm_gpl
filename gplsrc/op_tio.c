...
Private bool tio_print_string(PRINT_UNIT * pu, char * q, short int bytes, bool transparent_newline);
...
void op_at()
{
   ...
   switch(at1)
    {
     ...
     case IT_CUB:      /* @(-9,n)  -  Cursor backspace */
        if (at2 < 1) at2 = 1;
        if (at2 > tio.dsp.width) at2 = tio.dsp.width;
     ...
     case IT_CUU:      /* @(-10, n)  -  Cursor up line */
        if (at2 < 1) at2 = 1;
        if (at2 > tio.dsp.lines_per_page) at2 = tio.dsp.lines_per_page;
 ...
 if (at1 >= 0) tio.dsp.flags &= ~PU_PAGINATE;
}
 ...
 if (tio.terminfo_colour_map == NULL)
  {
   ...
   if (tio.terminfo_colour_map == NULL) tio.terminfo_colour_map = null_string;
  }

 for(p = tio.terminfo_colour_map, i = 0; i < user_colour && (*p != '\0'); p++, i++)
  {
   p = strchr(p, '|');
   if (p == NULL) return user_colour;
  }
 
 if ((*p == '\0') || (*p == '|')) return user_colour;
...
void op_break()
{
 ...
 if (arg->data.value)       /* BREAK ON */
  {
   if (process.break_inhibits) process.break_inhibits--;
  }
 else                       /* BREAK OFF */
  {
...
//0134 if (cproc_level > pu->level) pu->level = cproc_level;
 ...
 if ((pu->heading != NULL) || (pu->footing != NULL)) pu->flags |= PU_PAGINATE;
 else pu->flags &= ~PU_PAGINATE;
 ...
 */
 ...
 if (!(pu->flags & PU_OUTPUT)) eject = TRUE; /* 0382 Force page start if first output */
...
*/
 ...
 if (flags & IN_TIMEOUT) timeout_descr = *(--e_stack);
 else
  {
 ...
 if (flags & IN_MASK) mask_descr = *(--e_stack);
 else InitDescr(&mask_descr, UNASSIGNED);
...
void op_keyedit() {keyedit_trap_exit(0);}
void op_keyexit() {keyedit_trap_exit(1);}
void op_keytrap() {keyedit_trap_exit(2);}
...
void op_keyint()
{
 ...
 descr = e_stack-1;
 GetNum(descr);
 if (descr->type == INTEGER) timeout = descr->data.value * 1000;
 else timeout = (int)(descr->data.float_value * 1000);

 if (timeout < 0) timeout = 0;  /* 0145 */
   ...
   int timeout)  /* -ve = none */
{
 ...
 if (connection_type == CN_NONE) k_error(sysmsg(1701));
 if (is_QMVbSrvr) k_error(sysmsg(1702));

 k = keyin((timeout<0)?0:timeout);
 if ((k == 0) && (k_exit_cause & K_INTERRUPT))
  {      /* Break key, etc.  Process and then repeat */
   pc--;
   return;
  }

 if (timeout >= 0) k_pop(1);   /* 0145 Remove timeout from the stack */
 ...
 if (k < 0)   /* Exception condition (timeout or pipe eof) */
  ...
  {
   c = (char)k;
   if (invert && case_inversion && IsAlpha(c)) c ^= 0x20;
...
// 0184 pu->paginate = TRUE;
 ...
 if (descr->data.value >= 0) pu->page_no = (short int)(descr->data.value); /* 0382 moved */
...
//0134  }
 ...
 switch(param)
  {
   case LPTR_WIDTH:
      if (new_value == -1) pu->width = pcfg.lptrwide;
      else if (new_value >= 0) pu->width = (short int)new_value;
      new_value = pu->width;
      break;
      
   case LPTR_LINES:
      if (new_value == -1) pu->lines_per_page = pcfg.lptrhigh;
      else if (new_value >= 0) pu->lines_per_page = (short int)new_value;
      new_value = pu->lines_per_page;
      set_data_lines(pu);
      break;
      
   case LPTR_TOP_MARGIN:
      if (new_value == -1) pu->top_margin = TIO_DEFAULT_TMGN;
      else if (new_value >= 0) pu->top_margin = (short int)new_value;
      new_value = pu->top_margin;
      set_data_lines(pu);
      break;
      
   case LPTR_BOTTOM_MARGIN:
      if (new_value == -1) pu->bottom_margin = TIO_DEFAULT_BMGN;
      else if (new_value >= 0) pu->bottom_margin = (short int)new_value;
      new_value = pu->bottom_margin;
      set_data_lines(pu);
      break;
      
   case LPTR_LEFT_MARGIN:
      if (new_value == -1) pu->left_margin = TIO_DEFAULT_LMGN;
      else if (new_value >= 0) pu->left_margin = (short int)new_value;
   ...
   case LPTR_FLAGS:
      if (new_value == -1) pu->flags &= ~PU_USER_SETABLE;
      else if (new_value >= 0)
       {
   ...
   case LPTR_LINES_LEFT:          /* Query only - Lines remaining on page */
      ...
      if (pu->unit < 0) new_value--;  /* Space for pagination prompt */
      break;

   case LPTR_COPIES:              /* Number of copies */
      if (new_value == -1) pu->copies = 1;
      else if (new_value >= 0) pu->copies = (short int)new_value;
      new_value = pu->copies;
      break;
...
//0134          if (cproc_level > pu->level) pu->level = cproc_level;
   ...
   case PU_WIDTH:
      GetInt(descr);
      if ((n = (short int)(descr->data.value)) < 0) n = pcfg.lptrwide;
      pu->width = n;
      break;

   case PU_LENGTH:
      GetInt(descr);
      if ((n = (short int)(descr->data.value)) < 0) n = pcfg.lptrhigh;
      pu->lines_per_page = n;
      set_data_lines(pu);
      break;
   ...
   case PU_TOPMARGIN:
      GetInt(descr);
      if ((n = (short int)(descr->data.value)) < 0) n = TIO_DEFAULT_TMGN;
      pu->top_margin = n;
      set_data_lines(pu);
      break;
   ...
   case PU_BOTMARGIN:
      GetInt(descr);
      if ((n = (short int)(descr->data.value)) < 0) n = TIO_DEFAULT_BMGN;
      pu->bottom_margin = n;
      set_data_lines(pu);
      break;
   ...
   case PU_LEFTMARGIN:
      GetInt(descr);
      if ((n = (short int)(descr->data.value)) < 0) n = TIO_DEFAULT_LMGN;
      pu->left_margin = n;
      break;
      ...
      switch(pu->mode)
       {
        ...
        case PRINT_AND_HOLD:
           if (!validate_printer(printer_name))
            {
             if (printer_name != NULL) k_free(printer_name);
             process.status = ER_NOT_FOUND;
             break;
            }
           if (pu->printer_name != NULL) k_free(pu->printer_name);
           pu->printer_name = printer_name;
           break;

        case PRINT_TO_FILE:
           if (pu->file_name != NULL) k_free(pu->file_name);
   ...
   case PU_COPIES:
      GetInt(descr);
      if ((n = (short int)(descr->data.value)) < 1) n = 1;
      pu->copies = n;
      break;
   ...
   case PU_CPI:
      k_get_float(descr);
      pu->cpi = (short int)(descr->data.float_value * 100);  /* Stored scaled by 100 */
      break;
   ...
   switch(key)   /* !!PU!! */
    {
     ...
     case PU_LINESLEFT:
        ...
        if (pu->unit < 0) n--;  /* Space for pagination prompt */
        result.data.value = n;
        break;
     ...
     case PU_LINENO:
        ...
        if (n > pu->data_lines_per_page) n = 1;  /* Page throw pending */
        result.data.value = n;
        break;
 ...
 */
 ...
 switch(low_unit)
  {
   case -3:         /* Close all units at this or higher level */
      if (!(process.program.flags & HDR_INTERNAL)) goto exit_op_prclose;
 ...
 for (pu = &tio.dsp; pu != NULL; pu = pu->next)
  {
   if ((pu->unit >= low_unit) && (pu->unit <= high_unit)
       && (pu->level >= level))
    {
     if (!(pu->flags & PU_KEEP_OPEN)) pu->flags &= ~PU_PART_CLOSED;
 ...
 */
 ...
 if (pu == NULL) status = -ER_PRT_UNIT;
 else
  {
   if (pu->unit < 0) k_error(sysmsg(1704));
...
//0134 if (cproc_level > pu->level) pu->level = cproc_level;
 ...
 if (validate_printer(printer_name))
  ...
  {
   if (printer_name != NULL) k_free_ptr(printer_name);
   status = 1;
  }
 ...
 */
 ...
 if (pu->level == 0) pu->level = cproc_level;
...
void op_prompt()
{
 ...
 if (str != NULL) tio.prompt_char = str->data[0];
 else tio.prompt_char = '\0';
 ...
 switch(action)
  {
   case PT_BREAK:    /* Trap ctrl-c as break? */
      GetInt(descr);
      new_value = descr->data.value;
      if (new_value >= 0) set_term((new_value != 0));
   ...
   case PT_INVERT:   /* Case inversion */
      GetInt(descr);
      new_value = descr->data.value;
      if (new_value >= 0) case_inversion = (new_value != 0);
   ...
   case PT_BRKCH:    /* Set break character */
      GetInt(descr);
      new_value = descr->data.value;
      if (new_value != 0) tio.break_char = (char)new_value;
   ...
   case PT_ONEWLINE:  /* Set output newline sequence */
      ...
      if (str != NULL) k_get_c_string(descr, onewline, 10);
   ...
   case PT_INEWLINE:  /* Set input newline character */
      k_get_string(descr);
      str = descr->data.str.saddr;
      if (str != NULL) inewline = str->data[0];
   ...
   case PT_BINARY_IN:   /* Telnet binary mode, client -> server */
      GetInt(descr);
      new_value = descr->data.value;
      if (new_value >= 0) telnet_binary_mode_in = (new_value != 0);
   ...
   case PT_BINARY_OUT:  /* Telnet binary mode, server -> client */
      GetInt(descr);
      new_value = descr->data.value;
      if (new_value >= 0) telnet_binary_mode_out = (new_value != 0);
   ...
   case PT_TELNET:     /* Recognise TN_IAC on input */
      GetInt(descr);
      new_value = descr->data.value;
      if (new_value >= 0) telnet_negotiation = (new_value != 0);
 ...
 */
 ...
 if ((connection_type == CN_NONE) || is_QMVbSrvr) phantom_error();
 ...
 descr = e_stack - 2;
 k_get_value(descr);
 if (descr->type != IMAGE) k_error(sysmsg(1708));
 ...
 if (!tio.hush)
  {
   ...
   if (restore_all)
    {
     if (screen->pagination) tio.dsp.flags |= PU_PAGINATE;
 ...
 */
 ...
 if ((x < 0) || (width < 1) || ((x + width) > tio.dsp.width)
  || (y < 0) || (height < 1) || ((y + height) > tio.dsp.lines_per_page))
  {
   k_error(sysmsg(1709));
  }

 p = (SCREEN_IMAGE *)k_alloc(7, sizeof(struct SCREEN_IMAGE));
 if (p == NULL) k_error(sysmsg(1710));
...
void freescrn(SCREEN_IMAGE * image)
{
 ...
 if (image->buffer != NULL) k_free(image->buffer);
 k_free(image);
}
...
bool settermtype(char * name)
{
 ...
 if (!tsettermtype(name)) return FALSE;
...
Private int como_open(char name[])
{
 if (ValidFileHandle(tio.como_file)) como_close();

 tio.como_file = dio_open(name, DIO_REPLACE);
 if (!ValidFileHandle(tio.como_file)) return ER_FNF;
 ...
 if (to_display)
  {
   ...
   if (descr->data.value & 0x0001) tio.dsp.flags &= ~PU_PAGINATE;
  }
...
Private bool print_chunked_string(
   PRINT_UNIT * pu,
   bool flush)
{
 bool status = TRUE;
...
Private void set_data_lines(PRINT_UNIT * pu)
{
 ...
 if (pu->lines_per_page == 0) data_lines = 0;
 else data_lines = pu->lines_per_page - (pu->top_margin + pu->bottom_margin);
 ...
 if (pu->heading != NULL)
  {
   ...
   for (q = pu->heading, control_mode = FALSE; *q != '\0'; q++)
    {
     if (*q == '\'') control_mode = !control_mode;
   ...
   if (pu->flags & PU_ACTIVE)
    {
     ...
     if (pu->unit != -1) pu->flags &= ~(PU_ACTIVE | PU_OUTPUT);
    }
...
void squeek()
{
 ...
 if (descr->data.str.saddr != NULL) tio_printf("\x07");
}
 ...
 if (tio.dsp.flags & PU_HDR_NEXT) /* Force page heading */
  {
   ...
   if (tio.dsp.heading != NULL) display_header();
  }

 if (tio.hush) return TRUE;       /* Suppress screen and como output */
 
 if (!capturing)
  {
   if (tio.como_file >= 0) como_string(q, bytes);

   if (is_QMVbSrvr) return TRUE;  /* Ignore output from QMServer */ 

   if (connection_type == CN_NONE) return TRUE;
  }
...
{
 if (tio.dsp.line && (tio.dsp.flags & PU_PAGINATE)) display_footer();
 ...
 if (!capturing)
  {
   ...
   if (!Option(OptNoUserAborts)) tio_printf(", %s", sysmsg(1751));  /* A option */
   ...
   if (ValidFileHandle(tio.como_file)) como_string(Newline, NewlineBytes);
   tio_new_line();
  }
...
Private void display_header()
{
 ...
 if (tio.dsp.heading_lines) emit_header_footer(&tio.dsp, TRUE);
 flush_outbuf();
...
Private PRINT_UNIT * tio_find_printer(
   DESCRIPTOR * descr,     /* Print unit descriptor */
                              ...
                              If not set, interprets unit 0 as unit -1 */
{
 ...
 if (u == last_pu) return last_pu_ptr;
 ...
 switch(u)
  {
   ...
   default:
      if ((u < LOW_PRINT_UNIT) || (u > HIGH_PRINT_UNIT))
       {
        if (u != TEMPLATE_PRINT_UNIT) return NULL;
       }
 ...
 pu = tio_set_printer(u, PRINT_TO_FILE, pcfg.lptrhigh, pcfg.lptrwide,
                    TIO_DEFAULT_TMGN, TIO_DEFAULT_BMGN, TIO_DEFAULT_LMGN);
 last_pu = u;
 last_pu_ptr = pu;
 return pu;
}
...
bool tio_handle_break()
{
 ...
 if (image == NULL) k_error(sysmsg(1710));
...
/* A to abort, Q to quit, G to continue, D to debug, X to exit, ? for help */
     ...
     if (check_debug()) tio_printf("%s, ", sysmsg(1755));             /* D option */
...
bool tio_init()
{
 ...
 tio_set_printer(0, PRINT_TO_PRINTER,
                 pcfg.lptrhigh, pcfg.lptrwide, 0, 0, 0);
 ...
 if (tio.dsp.flags & PU_PAGINATE)
  {
   if (tio.dsp.line == (tio.dsp.data_lines_per_page - 1))
    {
     if (!displaying_footer) display_footer();
    }
  }
}
 ...
 if (unit != TEMPLATE_PRINT_UNIT)
  {
   for (pu = &tio.dsp; pu != NULL; pu = pu->next)
    {
     if (pu->unit == TEMPLATE_PRINT_UNIT) break;
    }
  }
 ...
 if (!in_debugger)
  {
   if (process.break_inhibits
      || (recursion_depth && !(process.program.flags & HDR_ALLOW_BREAK))
      )
    {
     break_pending = TRUE;
    }
   else k_exit_cause = K_QUIT;
  }
}
 ...
 */
 ...
 if (!read_socket((char *)&packet_bytes, 4)) goto exit_op_readpkt;
 if (process.status == ER_EOF) goto exit_op_readpkt;
 ...
 while(packet_bytes > 0)
  {
   ...
   if (!read_socket(str->data, n)) goto exit_op_readpkt;
 ...
 */
 ...
 packet_length = 4;
 if (str != NULL) packet_length += str->string_len;
 ...
 if (!write_socket((char *)&packet_length, 4, FALSE)) goto exit_op_writepkt;

/* Send packet data */

 while(str != NULL)
  {
   if (!write_socket(str->data, str->bytes, FALSE)) goto exit_op_writepkt;
...
Private void process_client_input()
{
 ...
 if (capturing)    /* 0254 Reworked */
  {
   ...
   if (capture_head != NULL) header_packet.packet_len += capture_head->string_len;
 ...
 if (!read_socket((char *)&in_packet, IN_PKT_SIZE)) return; /* Socket closed */
 ...
 while(packet_bytes > 0)
  {
   ...
   n = min(chunk_bytes, packet_bytes);
   if (!read_socket(str->data, n)) break;
 ...
 switch(connection_type)
  {
   case CN_SOCKET:
      if (is_QMVbSrvr)                /* Do not do TN parameter mangling */
       {
        if (!to_outbuf(str, bytes)) return FALSE;
       }
      else
       {
        while(bytes)
         {
          ...
          if (n)
           {
            if (!to_outbuf(str, n)) return FALSE;
            bytes -= n;
            str += n;
           }
          ...
          switch(mode)
           {
            case WS_CHAR_FF:
               if (!to_outbuf("\xff\xff", 2)) return FALSE;    /* 0264 */
               bytes--;
               str++;
               break;

            case WS_CR:
               if (!to_outbuf("\r\x00", 2)) return FALSE;
               bytes--;
               str++;
               break;
           }
      ...
      if (flush)
       {
        if (!flush_outbuf()) return FALSE;
       }
      break;

   case CN_PIPE:
   case CN_PORT:
      if (write(1, str, bytes) != bytes) return FALSE;   /* 0349 */
      break;
  }
...
int help(char * key)
{
 ...
 if (is_QMTerm)
  ...
  {
   p = qmtgetstr("u8");
   if (*p == '\0') return ER_TERMINFO;     /* No command code binding */
   strcpy(cmd, tparm(&n, p, cmd));
  }

 if (connection_type == CN_CONSOLE) tio_write(cmd);  /* 0404 */
 else write_socket(cmd, strlen(cmd), TRUE);
 ...
 for(prg = &process.program; prg != NULL; prg = prg->prev)
  {
   ...
   if (prg->flags & HDR_IS_CPROC) break;
  }
}
